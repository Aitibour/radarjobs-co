"""
POST /scan â€” accepts CV text + job title + location, scrapes jobs, scores them
...
def _upsert_results(
    ...
    Called in a fire-and-forget fashion â€” exceptions are caught and logged.
    """
    try:
        ...
        logger.warning("scan: DB upsert failed (non-fatal) â€” %s", exc)
...
async def run_scan(
    ...
    """
    Full pipeline: parse CV â†’ scrape jobs â†’ score each job â†’ save â†’ return.
    ...
    Hard timeout: 90 s â€” returns 504 instead of dropping the connection.
    """
...
async def _run_scan_pipeline(
    ...
    try:
        ...
        logger.error("scan: cv_parser failed â€” %s", exc)
    ...
    try:
        ...
        logger.error("scan: scraper failed â€” %s", exc)
    ...
    try:
        ...
        logger.error("scan: matcher failed â€” %s", exc)
...
async def extract_cv(request: ExtractRequest) -> ExtractResponse:
    ...
    No job scraping â€” fast response.
    """
    if not request.cv_text.strip():
        raise HTTPException(status_code=422, detail="cv_text must not be empty")
    ...
    try:
        ...
        logger.error("extract: cv_parser failed â€” %s", exc)
        raise HTTPException(status_code=500, detail="Failed to parse CV") from exc
...
Return ONLY the enhanced CV text. No preamble, no commentary."""
    try:
        ...
        logger.error("enhance_cv: failed â€” %s", exc)
