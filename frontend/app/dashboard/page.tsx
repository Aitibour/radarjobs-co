  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-10">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
          <h1 className="text-3xl font-extrabold text-teal-dark">
            Welcome back{user?.email ? `, ${user.email.split('@')[0]}` : ''}!
          </h1>
          <p className="text-gray-500 mt-1 text-sm">
            Last scanned:{' '}
            <span className="font-medium text-gray-700">{getLastScannedLabel()}</span>
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="border-2 border-teal-mid text-teal-mid font-semibold px-4 py-2.5 rounded-xl hover:bg-teal-light transition-colors text-sm"
          >
            {showAlertForm ? 'Hide Alerts' : 'Update Alert Preferences'}
          </button>
          <Link
            href="/scan"
            className="bg-teal-dark text-white font-bold px-5 py-2.5 rounded-xl hover:bg-teal-mid transition-colors text-sm inline-flex items-center gap-1.5"
          >
            New Scan →
          </Link>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-10">
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          <p className="text-gray-400 text-xs uppercase tracking-wider mb-1">Total matches</p>
          <p className="text-3xl font-extrabold text-teal-dark">{matches.length}</p>
        </div>
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          <p className="text-gray-400 text-xs uppercase tracking-wider mb-1">Top score</p>
          <p className="text-3xl font-extrabold text-teal-mid">
