...
export default function LoginPage() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    if (error) { setError(error.message); setIsLoadingOAuth(false) }
  }
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden bg-teal-light">
      {/* Back link */}
      <div className="shrink-0 px-6 py-3">
        <Link href="/" className="text-sm text-teal-dark font-medium hover:underline">← Back to home</Link>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center px-4 overflow-y-auto">
        <div className="w-full max-w-md">
          <div className="bg-white rounded-3xl shadow-xl p-8 md:p-10">
            ...
            <h1 className="text-2xl font-extrabold text-gray-900 text-center mb-1">Sign in</h1>
            <p className="text-gray-500 text-sm text-center mb-6">Save your CV, track matches, get alerts.</p>

            {magicLinkSent ? (
              <div className="bg-teal-light border border-teal-accent rounded-2xl p-6 text-center">
                <div className="w-12 h-12 bg-teal-mid/10 rounded-full flex items-center justify-center mx-auto mb-3">
                ...
                <p className="text-gray-600 text-sm">Magic link sent to <span className="font-medium text-teal-dark">{email}</span></p>
                <button onClick={() => setMagicLinkSent(false)} className="mt-3 text-sm text-teal-mid hover:underline">
              ...
              <>
                <button
                  ...
                  className="w-full flex items-center justify-center gap-3 border-2 border-gray-200 rounded-xl py-3 px-4 font-semibold text-gray-700 hover:border-teal-mid hover:bg-teal-light transition-all disabled:opacity-60 disabled:cursor-not-allowed mb-4"
                >
                  {isLoadingOAuth ? (
                ...
                <form onSubmit={handleMagicLink} className="space-y-3">
                  <input
                    ...
                    required
                    className="w-full border-2 border-gray-200 rounded-xl px-4 py-3 text-gray-900 placeholder-gray-400 focus:outline-none focus:border-teal-mid transition-colors"
                  />
                  <button
                    type="submit"
                    disabled={isLoadingMagic || !email.trim()}
                    className="w-full bg-teal-dark text-white font-bold py-3 rounded-xl hover:bg-teal-mid transition-colors disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                  >
                    {isLoadingMagic ? (
